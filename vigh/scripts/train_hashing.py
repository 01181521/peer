...
#                 np.save(config["save_path"] + str(nbit) + "-" + "trn_binary.npy", db_out['codes'].sign().cpu().numpy())
#                 np.save(config["save_path"] + str(nbit) + "-" + "tst_binary.npy", test_out['codes'].sign().cpu().numpy())
#                 np.save(config["save_path"] + str(nbit) + "-" + "trn_label.npy", db_out['labels'].sign().cpu().numpy())
#                 np.save(config["save_path"] + str(nbit) + "-" + "tst_label.npy", test_out['labels'].sign().cpu().numpy())
#                 np.save(config["save_path"] + str(nbit) + "-" + "map.npy",best)
#                 # savemat(config["save_path"] +str(nbit) +'map.mat', {'data':np.array(best.cpu())})
#                 savemat(config["save_path"] +str(nbit) +'db.mat', {'data':np.array(db_out['codes'].detach().sign().cpu()),'label':np.array(db_out['labels'].detach().sign().cpu())})
#                 savemat(config["save_path"] +str(nbit) +'test.mat', {'data':np.array(test_out['codes'].detach().sign().cpu()),'label':np.array(test_out['labels'].detach().sign().cpu())})
...
def train_hashing(optimizer, model, codebook, train_loader, loss_param):
    ...
    for i, (data, labels,ind,target) in enumerate(train_loader):
        timer.tick()
        ...
        loss = criterion(logits, codes, labels)#+con_loss(logits,target)##+0.0001*(codes.abs() - 1).pow(3).abs().mean()
        # +con_loss(logits,target)#2*
        # (1 - u.abs()).abs().mean()
        # (u.abs() - 1).pow(3).abs().mean()

        # (codes - codes.sign()).pow(2).mean()
        #+0.01*con_loss(logits,target)

        
#+0.0001*(codes - codes.sign()).pow(2).mean()
...
def test_hashing(model, codebook, test_loader, loss_param, return_codes=False):
    ...
    for i, (data, labels,s,d) in enumerate(test_loader):
        timer.tick()
...
def prepare_dataloader(config):
    logging.info('Creating Datasets new')
    train_loader, test_loader, db_loader, num_train, num_test, num_dataset = get_data(config)
...
def main(config):
    ...
    configs.seeding(int(config['seed']))
